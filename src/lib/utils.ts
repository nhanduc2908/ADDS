...
export function exportToXML(assessment: Assessment, criteria: SecurityCriterion[]): string {
  const lines = ['<?xml version="1.0" encoding="UTF-8"?>'];
  lines.push('<securityAssessment>');
  lines.push(`  <id>${escapeXml(assessment.id)}</id>`);
  lines.push(`  <name>${escapeXml(assessment.name)}</name>`);
  lines.push(`  <createdAt>${assessment.createdAt}</createdAt>`);
  lines.push(`  <updatedAt>${assessment.updatedAt}</updatedAt>`);
  lines.push(`  <overallScore>${assessment.overallScore}</overallScore>`);
  lines.push('  <results>');
  
  for (const result of assessment.results) {
    const c = criteria.find((cr) => cr.id === result.criterionId);
    if (!c) continue;
    lines.push('    <result>');
    lines.push(`      <criterionId>${result.criterionId}</criterionId>`);
    lines.push(`      <code>${escapeXml(c.code)}</code>`);
    lines.push(`      <name>${escapeXml(c.nameVi)}</name>`);
    lines.push(`      <riskLevel>${c.riskLevel}</riskLevel>`);
    lines.push(`      <score>${result.score}</score>`);
    lines.push(`      <status>${result.status}</status>`);
    lines.push(`      <notes>${escapeXml(result.notes)}</notes>`);
    lines.push(`      <assessedAt>${result.assessedAt}</assessedAt>`);
    lines.push(`      <assessedBy>${escapeXml(result.assessedBy)}</assessedBy>`);
    lines.push('    </result>');
  }
...
export function exportToMarkdown(assessment: Assessment, criteria: SecurityCriterion[]): string {
  const lines: string[] = [];
  lines.push(`# Báo Cáo Đánh Giá Bảo Mật`);
  lines.push('');
  lines.push(`**Tên đánh giá:** ${assessment.name}`);
  lines.push(`**Ngày tạo:** ${new Date(assessment.createdAt).toLocaleDateString('vi-VN')}`);
  lines.push(`**Điểm tổng:** ${assessment.overallScore}%`);
  lines.push('');
  lines.push('## Tổng quan');
  lines.push('');
  lines.push(`| Chỉ tiêu | Giá trị |`);
  lines.push(`|-----------|---------|`);
  lines.push(`| Tổng số tiêu chí | ${criteria.length} |`);
  lines.push(`| Số tiêu chí đã đánh giá | ${assessment.results.filter(r => r.status !== 'not-assessed').length} |`);
  lines.push(`| Điểm tổng | ${assessment.overallScore}% |`);
  lines.push('');
  lines.push('## Chi tiết đánh giá');
  lines.push('');
  lines.push('| Mã | Tiêu chí | Mức độ | Điểm | Trạng thái | Ghi chú |');
  lines.push('|----|----------|--------|------|-------------|---------|');
  
  for (const result of assessment.results) {
    const c = criteria.find((cr) => cr.id === result.criterionId);
    if (!c) continue;
    const statusIcon = result.status === 'compliant' ? '✅' : result.status === 'partial' ? '⚠️' : result.status === 'non-compliant' ? '❌' : '➖';
    lines.push(`| ${c.code} | ${c.nameVi} | ${c.riskLevel} | ${result.score}/5 | ${statusIcon} ${result.status} | ${result.notes || '-'} |`);
  }
  
  lines.push('');
  lines.push('---');
  lines.push(`*Generated by Security Audit System on ${new Date().toLocaleString('vi-VN')}*`);
  return lines.join('\n');
}

export function exportToYAML(assessment: Assessment, criteria: SecurityCriterion[]): string {
  const lines: string[] = [];
  lines.push('security_assessment:');
  lines.push(`  id: "${assessment.id}"`);
  lines.push(`  name: "${escapeYaml(assessment.name)}"`);
  lines.push(`  created_at: "${assessment.createdAt}"`);
  lines.push(`  updated_at: "${assessment.updatedAt}"`);
  lines.push(`  overall_score: ${assessment.overallScore}`);
  lines.push('');
  lines.push('  results:');
  ...
  for (const result of assessment.results) {
    const c = criteria.find((cr) => cr.id === result.criterionId);
    if (!c) continue;
    lines.push('    - criterion_id: "' + result.criterionId + '"');
    lines.push(`      code: "${escapeYaml(c.code)}"`);
    lines.push(`      name: "${escapeYaml(c.nameVi)}"`);
    lines.push(`      risk_level: "${c.riskLevel}"`);
    lines.push(`      score: ${result.score}`);
    lines.push(`      status: "${result.status}"`);
    lines.push(`      notes: "${escapeYaml(result.notes)}"`);
    lines.push(`      assessed_at: "${result.assessedAt}"`);
    lines.push(`      assessed_by: "${escapeYaml(result.assessedBy)}"`);
    lines.push('');
  }
